...
public class SortListUtil {
	...
	/**
	 * list的排序通用方法,针对int,long,double等字段进行排序
	 * @param list
	 * 		要排序的list
	 * @param method
	 * 		属性的get方法名
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <T> void sortByNumberField(List<T> list , final String method , final String sort){
		Collections.sort(list , new Comparator() {
			public int compare(Object o1 , Object o2){
				int res = 0;
				try {
					...
					Method method2 = ((T)o2).getClass().getMethod(method);
					if(sort != null && "desc".equals(sort)){
						res = DataFormatTool.stringToDouble(method2.invoke((T)o2).toString(), false) > DataFormatTool.stringToDouble(method1.invoke((T)o1).toString() ,false) ? 1 : -1;
					}else {
						res = DataFormatTool.stringToDouble(method2.invoke((T)o2).toString(), false) > DataFormatTool.stringToDouble(method1.invoke((T)o1).toString() ,false) ? -1 : 1;
					}
				} catch (Exception e) {
